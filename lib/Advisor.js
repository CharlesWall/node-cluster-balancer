...
function Advisor(options) {
    ...
    if (storage.type === 'ioredis') {
        storage.statusTTL = storage.statusTTL || _interval * 2;
        storage.clusterName = _clusterName;
        _dbInterface = require('./storage/ioredis').create(storage, this);
    } else if (storage.type === 'zookeeper') {
        storage.sessionTimeout = storage.sessionTimeout || _interval * 2;
        storage.clusterName = _clusterName;
        _dbInterface = require('./storage/zookeeper').create(storage, this);
    } else if (storage.type === 'custom' || storage.type == null) {
        _dbInterface = storage;
    } else {
        throw new Error(`Unknown storage type: ${storage.type}`);
    }

    this.ready = function() {
        return _dbInterface.ready().then(() => {
            return _advisor;
        });
    };
    ...
    this.stop = function() {
        ...
        _getStatusTimeoutId = null;
        return _dbInterface.stop();
    };
    ...
    this.getAllStatuses = function() {
        return Promise.all([
            ...
            let statuses = result[0];
            return statuses.concat(result[1]);
        });
    };
     ...
     */
    this.getLeastUtilizedTarget = () => {
        return this.getAllStatuses().then((statuses) => {
    ...
    function _getPeerStatuses(){
        return _dbInterface.getPeerStatuses().tap(function(peerStatuses) {
    ...
    function _generateAdvice(selfStatus, peerStatuses) {
        if (!peerStatuses.length) {
        ...
        peerStatuses.forEach(function(status) {
            ...
            if (!status || !_isTargetHealthy(status)) {
                return;
            }
