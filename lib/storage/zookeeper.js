...
function ZookeeperInterface(options) {
    ...
    this.getPeerStatuses = function() {
        ...
        return _getPeerList().then((children) => {
            ...
            return peerStatuses.filter((status) => {
                return (status != null);
            });
        });
    };
    ...
    this.ready = function() {
        ...
        return _clientPromise || _setupPromise || _setup();
    };
    ...
    function _setup() {
        ...
        _setupPromise = _setupPromise || _getClient()
            .tap((client) => {
                return client ? _ensureDirectory(client) : null;
            }).tap((client) => {
                return client ? _createSelfNode(client) : null;
            });
